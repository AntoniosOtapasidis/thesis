...
class Decoder_multiple_covariates(nn.Module):
    ...
    def __init__(self, output_dim, n_covariates,
        ...
        self.n_covariates_interactions = len(mappings_cz)
        ...
        self.Lambda_cz_1 = [
            Variable(lambda0*torch.ones(self.n_grid_z, output_dim, device=device), requires_grad=True) for _ in range(self.n_covariates)
        ]

        self.Lambda_cz_2 = [
            Variable(lambda0*torch.ones(self.n_grid_c[j], output_dim, device=device), requires_grad=True) for j in range(self.n_covariates)
        ]
    ...
    def forward_c(self, c):
        # Handle c as list of tensors by stacking into single tensor
    ...
    def forward_cz(self, z, c):
        ...
        if isinstance(c, (list, tuple)):
            c = torch.cat(c, dim=1)
        if self.n_covariates_interactions == 0:
            return []
        if self.has_feature_level_sparsity:
            out = []
            w = rsample_RelaxedBernoulli(self.temperature, self.qlogits_cz)
            for j in range(self.n_covariates_interactions):
                input = torch.cat([z, c[:, j:(j + 1)]], dim=1)
                out.append(w[j, :] * (self.mappings_cz[j](input)))
        else:
            out = [(self.mappings_cz[j](torch.cat([z, c[:, j:(j + 1)]], dim=1))) for j in range(self.n_covariates_interactions)]
    ...
    def calculate_integrals(self):
        ...
        int_cz_dc = [None] * self.n_covariates_interactions
        int_cz_dz = [None] * self.n_covariates_interactions
        ...
        for j in range(self.n_covariates_interactions):
            ...
            # out = self.mappings_cz[j](self.grid_cz[j])
    ...
    def fraction_of_variance_explained(self, z, c, account_for_noise=False, divide_by_total_var=True, Y_error=None):
        ...
        with torch.no_grad():
            ...
            if self.n_covariates_interactions != 0:
                ...
                f_int_vars = [torch.zeros(f_c_var.shape).to(self.device) for f_c_var in f_c_vars]
...
class Decoder_multiple_latents(Decoder_multiple_covariates):
    def forward_c(self, cs):
        return [self.mappings_c[j](cs[j]) for j in range(self.n_covariates)]