...
# - Bacterial community composition (via As1_shared)
...
cat(sprintf("Ws = random Gaussian: %d x %d\n", nrow(Ws), ncol(Ws)))
...
for (sim in 1:n_sims) {
  ...
  # =================================================================
  # GENERATE X1, X2: Latents -> log-concentrations -> compositions
  # =================================================================
  BV <- compute_BV(
    Z1_s = Z1_s,
    Z2_s = Z2_s,
  ...
  # =================================================================
  # Instead of fixed depths:
  depth_bac <- 10000
  depth_vir <- 15000

  # Use variable depths (e.g., log-normal distribution):
  depth_bac <- round(rlnorm(1, meanlog = log(10000), sdlog = 0.5))# The sdlog parameter controls the coefficient of variation
...
write.csv(Z1_df, "Z1_latents_RA_complex_sparse.csv", row.names = FALSE)
write.csv(Z2_df, "Z2_latents_RA_complex_sparse.csv", row.names = FALSE)
write.csv(Zs_df, "Zs_latents_RA_complex_sparse.csv", row.names = FALSE)
...
# Y_mat is in log space (metabolites)
# X1_mat and X2_mat are relative abundances (from multinomial sampling)
