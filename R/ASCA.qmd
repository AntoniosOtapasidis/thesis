...
##  Inspect the length of each column
...
# Confirm lmpData has the expected slots
names(lmpData)
names(lmpData_log)

# Confirm outcomes is numeric matrix and dimensions look right
...
#  Confirm your log transform actually happened
identical(lmpData$outcomes, lmpData_log$outcomes)   # should be FALSE

# Confirm there are no Inf/-Inf/NaN introduced
sum(!is.finite(lmpData_log$outcomes))
range(lmpData_log$outcomes, finite = TRUE)

#  Confirm you're calling the right PCA functions and result has expected fields
resPCA1 <- limpca::pcaBySvd(lmpData_log$outcomes)
class(resPCA1)
names(resPCA1)
