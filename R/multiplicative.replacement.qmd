...
# zCompositions expects samples in ROWS -> transpose
X_t <- t(X)
...
# Multiplicative replacement with CZM (δ = 0.65 × min positive when DL unknown)
...
# Transpose back to taxa x samples and re-close columns (guard against numeric drift)
X_rep <- t(X_rep_t)
